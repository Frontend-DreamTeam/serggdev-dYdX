(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        let mySwiper = new swiper_core_Swiper(".comments__slider", {
            navigation: {
                nextEl: ".swiper-button-next",
                prevEl: ".swiper-button-prev"
            },
        ...
        nextButton.addEventListener("click", (() => {
            if (!mySwiper.isEnd) mySwiper.slideNext();
        }));
        let prevButton = document.querySelector(".swiper-button-prev");
        prevButton.addEventListener("click", (() => {
            if (!mySwiper.isBeginning) mySwiper.slidePrev();
        }));
        mySwiper.on("slideChange", (() => {
            toggleButton(nextButton, mySwiper.isEnd);
            toggleButton(prevButton, mySwiper.isBeginning);
        }));
        function toggleButton(button, condition) {
            button.style.display = condition ? "none" : "block";
        }
    }));
    ...
    const observer = new IntersectionObserver((entries => {
        entries.forEach((entry => {
            if (entry.target.classList.contains("page__first-block")) {
                numCounter("#num-1", 100, 1400, 1);
                numCounter("#num-2", 3, 1e3, 1);
                numCounter("#num-3", 400, 1500, 10);
            }
        }));
    }));
    document.querySelectorAll(".page__first-block").forEach((section => {
